...
HIP_TYPED_TEST_P(ReduceHIP, generic)
{
  ...
  for (int tcount = 0; tcount < test_repeat; ++tcount) {
    ...
    for (int k = 0; k < loops; k++) {
      ...
      forall<hip_exec<block_size>>(RAJA::RangeSegment(0, TEST_VEC_LEN), [=] RAJA_DEVICE(int i) {
...
//
HIP_TYPED_TEST_P(ReduceHIP, indexset_align)
{
...
//
HIP_TYPED_TEST_P(ReduceHIP, indexset_noalign)
{
...
using MinLocTypes =
    ::testing::Types<ReduceMinLoc<hip_reduce<block_size>, double>>;
INSTANTIATE_TYPED_TEST_CASE_P(MinLoc, ReduceHIP, MinLocTypes);

using MaxLocTypes =
    ::testing::Types<ReduceMaxLoc<hip_reduce<block_size>, double>>;
