...
HIP_TEST_F(ReduceMinHIP, generic)
{
  ...
  for (int tcount = 0; tcount < test_repeat; ++tcount) {
    ...
    ReduceMin<hip_reduce<block_size>, double> dmin0; dmin0.reset(DEFAULT_VAL);
    ReduceMin<hip_reduce<block_size>, double> dmin1(DEFAULT_VAL);
    ReduceMin<hip_reduce<block_size>, double> dmin2(BIG_VAL);
...
//
HIP_TEST_F(ReduceMinHIP, indexset_align)
{
  ...
  for (int tcount = 0; tcount < test_repeat; ++tcount) {
    ...
    ReduceMin<hip_reduce<block_size>, double> dmin0(DEFAULT_VAL);
    ReduceMin<hip_reduce<block_size>, double> dmin1(DEFAULT_VAL);
...
//
HIP_TEST_F(ReduceMinHIP, indexset_noalign)
{
