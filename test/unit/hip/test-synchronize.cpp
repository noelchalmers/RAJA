...
HIP_TEST(SynchronizeTest, HIP){

  double* managed_data = (double*) malloc(sizeof(double)*50);
  double* d_managed_data;
  hipMalloc(&d_managed_data, sizeof(double)*50);

  RAJA::forall<RAJA::hip_exec_async<256>>(RAJA::make_range(0, 50), [=] RAJA_HOST_DEVICE (RAJA::Index_type i) {
  ...
  RAJA::forall<RAJA::loop_exec>(RAJA::make_range(0, 50), [=] (RAJA::Index_type i) {
