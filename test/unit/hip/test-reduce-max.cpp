...
HIP_TEST_F(ReduceMaxHIP, generic)
{
  ...
  for (int tcount = 0; tcount < test_repeat; ++tcount) {
    ...
    ReduceMax<hip_reduce<block_size>, double> dmax0; dmax0.reset(DEFAULT_VAL);
    ReduceMax<hip_reduce<block_size>, double> dmax1(DEFAULT_VAL);
    ReduceMax<hip_reduce<block_size>, double> dmax2(BIG_VAL);
...
//
HIP_TEST_F(ReduceMaxHIP, indexset_align)
{
  ...
  for (int tcount = 0; tcount < test_repeat; ++tcount) {
    ...
    ReduceMax<hip_reduce<block_size>, double> dmax0(DEFAULT_VAL);
    ReduceMax<hip_reduce<block_size>, double> dmax1(DEFAULT_VAL);
...
//
HIP_TEST_F(ReduceMaxHIP, indexset_noalign)
{
