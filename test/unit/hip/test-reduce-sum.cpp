...
HIP_TEST_F(ReduceSumHIP, staggered_sum)
{
  double* dvalue = ReduceSumHIP::d_dvalue;

  double dtinit = 5.0;

  ReduceSum<hip_reduce<block_size>, double> dsum0(0.0);
  ReduceSum<hip_reduce<block_size>, double> dsum1(dtinit * 1.0);
  ReduceSum<hip_reduce<block_size>, double> dsum2(0.0);
  ReduceSum<hip_reduce<block_size>, double> dsum3(dtinit * 3.0);
  ReduceSum<hip_reduce<block_size>, double> dsum4(0.0);
  ReduceSum<hip_reduce<block_size>, double> dsum5(dtinit * 5.0);
  ReduceSum<hip_reduce<block_size>, double> dsum6(0.0);
  ReduceSum<hip_reduce<block_size>, double> dsum7(dtinit * 7.0);

  int loops = 2;
  for (int k = 0; k < loops; k++) {

    forall<hip_exec<block_size> >(RangeSegment(0, TEST_VEC_LEN), [=] RAJA_HOST_DEVICE(int i) {
      dsum0 += dvalue[i];
      dsum1 += dvalue[i] * 2.0;
      dsum2 += dvalue[i] * 3.0;
      dsum3 += dvalue[i] * 4.0;
...
HIP_TEST_F(ReduceSumHIP, staggered_sum2)
{
  double* dvalue = ReduceSumHIP::d_dvalue;

  double dtinit = 5.0;

  ReduceSum<hip_reduce<block_size>, double> dsum0(5.0);
  ReduceSum<hip_reduce<block_size>, double> dsum1;
  ReduceSum<hip_reduce<block_size>, double> dsum2(5.0);
  ReduceSum<hip_reduce<block_size>, double> dsum3;
  ReduceSum<hip_reduce<block_size>, double> dsum4(5.0);
  ReduceSum<hip_reduce<block_size>, double> dsum5;
  ReduceSum<hip_reduce<block_size>, double> dsum6(5.0);
  ReduceSum<hip_reduce<block_size>, double> dsum7;
  ...
  for (int k = 0; k < loops; k++) {
    ...
    forall<hip_exec<block_size> >(RangeSegment(0, TEST_VEC_LEN), [=] RAJA_DEVICE(int i) {
      dsum0 += dvalue[i];
      dsum1 += dvalue[i] * 2.0;
      dsum2 += dvalue[i] * 3.0;
      dsum3 += dvalue[i] * 4.0;
...
HIP_TEST_F(ReduceSumHIP, indexset_aligned)
{
  double* dvalue = ReduceSumHIP::d_dvalue;
  ...
  ReduceSum<hip_reduce<block_size>, double> dsum0(dtinit * 1.0);
  ReduceSum<hip_reduce<block_size>, int> isum1(itinit * 2);
...
//
HIP_TEST_F(ReduceSumHIP, indexset_noalign)
{
  double* dvalue = ReduceSumHIP::d_dvalue;
  int* ivalue = ReduceSumHIP::d_ivalue;
...
HIP_TEST_F(ReduceSumHIP, atomic_reduce)
{
  double* rand_dvalue = ReduceSumHIP::rand_dvalue;
  double* d_rand_dvalue = ReduceSumHIP::d_rand_dvalue;

  ReduceSum<hip_reduce_atomic<block_size>, double> dsumN(0.0);
  ReduceSum<hip_reduce_atomic<block_size>, double> dsumP(0.0);
...
HIP_TEST_F(ReduceSumHIP, increasing_size)
{
  double* dvalue = ReduceSumHIP::d_dvalue;
  ...
  for (int size = block_size; size <= TEST_VEC_LEN; size+=block_size ) {
    ...
    ReduceSum<hip_reduce<block_size, true>, double> dsum0(dtinit);

    forall<hip_exec<block_size, true> >(RangeSegment(0, size), [=] RAJA_DEVICE(int i) {
      dsum0 += dvalue[i];
    });
