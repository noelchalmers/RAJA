...
#define HIP_TEST(X, Y)                 \
  static void hip_test_##X##_##Y();    \
  TEST(X, Y) { hip_test_##X##_##Y(); } \
  static void hip_test_##X##_##Y()

#define HIP_TEST_F(test_fixture, test_name)                  \
  static void hip_test_f_##test_fixture##_##test_name();     \
  GTEST_TEST_(test_fixture,                                   \
              test_name,                                      \
              test_fixture,                                   \
              ::testing::internal::GetTypeId<test_fixture>()) \
  {                                                           \
    hip_test_f_##test_fixture##_##test_name();               \
  }                                                           \
  static void hip_test_f_##test_fixture##_##test_name()

#define HIP_TEST_P(test_case_name, test_name)                               \
  template <typename Invocable>                                              \
  static void gtest_hip_##test_case_name##_##test_name(Invocable &&);       \
  class GTEST_TEST_CLASS_NAME_(test_case_name, test_name)                    \
      : public test_case_name                                                \
  {                                                                          \
  public:                                                                    \
    ...
    virtual void TestBody()                                                  \
    {                                                                        \
      gtest_hip_##test_case_name##_##test_name([&] { return GetParam(); }); \
    }                                                                        \
                                                                             \
  private:                                                                   \
    static int AddToRegistry()                                               \
  ...
  int GTEST_TEST_CLASS_NAME_(test_case_name,                                 \
      ...
      GTEST_TEST_CLASS_NAME_(test_case_name, test_name)::AddToRegistry();    \
  template <typename Invocable>                                              \
  static void gtest_hip_##test_case_name##_##test_name(Invocable &&GetParam)


#define HIP_TYPED_TEST_P(CaseName, TestName)                            \
  namespace GTEST_CASE_NAMESPACE_(CaseName)                              \
  {                                                                      \
    template <typename gtest_TypeParam_>                                 \
    class TestName : public CaseName<gtest_TypeParam_>                   \
