...
struct HipIndexCalc_Policy;


template <camp::idx_t ArgumentId>
struct HipIndexCalc_Policy<ArgumentId, seq_exec> {
  ...
  {
    return HipCarryPair{carry_init, carry_incr};
  }
...
template <camp::idx_t ArgumentId>
struct HipIndexCalc_Policy<ArgumentId, hip_thread_exec> {
  ...
  initThread(Data &data, int carry_thread, int carry_incr, bool final = false)
  {
    ...
    if(final && carry_thread >= len){
      ...
      return HipCarryPair{carry_out, full_cycle};
    }
  }
...
struct IndexCalcHelper {
  ...
  static RAJA_INLINE RAJA_HOST_DEVICE HipCarryPair initThread(Data &data,
                                                     CalcList &calc_list,
                                                     int carry_init,
                                                     int carry_incr)
  {
...
struct IndexCalcHelper<-1> {
  ...
  template <typename Data, typename CalcList>
  static RAJA_INLINE RAJA_HOST_DEVICE HipCarryPair initThread(Data &,
                                                     CalcList &,
                                                     int carry_init,
                                                     int carry_incr)
  {
...
struct HipIndexCalc<SegmentTuple,
 ...
 */
template <typename SegmentTuple>
struct HipIndexCalc<SegmentTuple, camp::list<>, camp::idx_seq<>> {
...
template <typename SegmentTuple>
using HipIndexCalc_Terminator =
    HipIndexCalc<SegmentTuple, camp::list<>, camp::idx_seq<>>;


template <typename IndexCalcBase, typename CalcN>
struct HipIndexCalc_Extender;
...
template <typename SegmentTuple,
          ...
          camp::idx_t... RangeInts,
          typename CalcN>
struct HipIndexCalc_Extender<HipIndexCalc<SegmentTuple,
                                            camp::list<CalcPolicies...>,
                                            camp::idx_seq<RangeInts...>>,
                              CalcN> {
  using type = HipIndexCalc<SegmentTuple,
                             camp::list<CalcPolicies..., CalcN>,
                             camp::idx_seq<RangeInts..., sizeof...(RangeInts)>>;
};

template <typename IndexCalcBase, typename CalcN>
using ExtendHipIndexCalc =
    typename HipIndexCalc_Extender<IndexCalcBase, CalcN>::type;
...
struct HipStatementListExecutorHelper {
  ...
  using next_helper_t =
      HipStatementListExecutorHelper<cur_stmt + 1, num_stmts>;
...
struct HipStatementListExecutorHelper<num_stmts, num_stmts> {
...
struct HipStatementListExecutor<Data, StatementList<Stmts...>, IndexCalc> {

  using enclosed_stmts_t =
      camp::tuple<HipStatementExecutor<Data, Stmts, IndexCalc>...>;
                              ...
                              int block_carry)
  {
    ...
    HipStatementListExecutorHelper<0, sizeof...(Stmts)>::exec(
                                    ...
                                    int block_stride)
  {
    ...
    // Execute statements in order with helper class
    HipStatementListExecutorHelper<0, sizeof...(Stmts)>::initBlocks(
  ...
  inline RAJA_DEVICE void initThread(Data &data)
  {
    ...
    // Execute statements in order with helper class
    HipStatementListExecutorHelper<0, sizeof...(Stmts)>::initThread(
        enclosed_stmts, data);
  }
  ...
  LaunchDim calculateDimensions(Data const &data, LaunchDim const &max_physical)
  {
    ...
    // Compute this statements launch dimensions
    return HipStatementListExecutorHelper<0, sizeof...(Stmts)>::
